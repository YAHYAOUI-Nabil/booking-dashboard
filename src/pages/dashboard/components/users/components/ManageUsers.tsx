...
import { useDeleteUserMutation } from "../../../../../app/api/private/user";

interface ManageUsersProps {
  users?: Admin[];
  selectedRow: string;
  handleSelectRow: (index: string) => void;
}
...
const ManageUsers: React.FC<ManageUsersProps> = ({
  ...
  const handleDeleteUser = (id: string) => {
    ...
    if (selectedRow !== id) {
      toast.error("Please select the user you want to delete");
      return;
    }

    openDialog(
      async () => await deleteUser({ id }).unwrap(),
      "Are you sure you want to delete the selected user?"
    );
  };

  const renderUsers = users?.map((user: Admin) => (
    <tr
      key={user._id}
      className={`transition-all h-[54px] min-w-[800px] duration-300 ease-in-out mb-1 hover:bg-[#FBF9FF] overflow-hidden table-row ${
        selectedRow === user._id ? "bg-[#FBF9FF]" : ""
      }`}
  ...
  return (
    <div className="sm:rounded-lg w-full">
      ...
      >
        <thead className=" bg-white w-full table-header-group">
          <tr className="text-black items-center rounded-md h-[39px] ">
            <th className="rounded-l w-[59px]">
              <div className="flex items-center justify-end pl-[5px] gap-[6px] pr-2"></div>
            </th>
